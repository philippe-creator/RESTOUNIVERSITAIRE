#!/usr/bin/env pwsh
# Script de lancement complet - RESTO_BM avec Chat Vocal
# ======================================================
# Utilisation: .\lancer.ps1
# 
...
Start-Process powershell.exe -ArgumentList "-NoExit", "-Command", "& '$backendPath'" `
...
Start-Process powershell.exe -ArgumentList "-NoExit", "-Command", "& '$apiPath'" `
  -WindowStyle Normal -PassThru | Out-Null
...
Start-Process powershell.exe -ArgumentList "-NoExit", "-Command", "& '$frontendPath'" `
  -WindowStyle Normal -PassThru | Out-Null
